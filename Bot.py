...
def ai_route_and_reply(user: Dict, history: List[Dict], text: str) -> Tuple[str, Optional[Dict]]:
    ...
    sys = (
        ...
        "• Если у пользователя прямой вопрос, который не требует действия — action='answer' и краткий текст в reply.\n"
        "• 'price' — показать котировки (args: {tickers:[\"AAPL\",\"BTC\"]} или пусто — взять список пользователя).\n"
        "• 'chart' — график (args: {ticker:\"BTC\", period:\"7d|30d|90d|1y\"}, по умолчанию '7d').\n"
        "• 'convert' — конвертация (args: {amount:число, frm:\"USD\", to:\"RUB\"}).\n"
        "• 'add'/'remove' — добавление/удаление тикеров (args: {tickers:[...]}).\n"
        "• 'setbase' — смена базовой валюты (args: {base:\"USD|EUR|RUB|GBP|JPY|CHF|CNY|AUD|CAD|TRY\"}).\n"
        "• 'analyze' — обзор/аналитика (args: {tickers:[...], horizon:\"7d|30d|90d|1y\"}).\n"
        "Никаких инвестиционных советов. Краткость. Только коды валют (USD, EUR ...)."
    )
...
def main_menu_markup(user: Dict) -> InlineKeyboardMarkup:
    ...
    rows = [
        ...
        [InlineKeyboardButton("🤖 Помощник", callback_data="ACT:ASSIST")],
        [InlineKeyboardButton("🧠 Аналитик", callback_data="ACT:ANALYST")],
    ]
...
def ai_generate_insights(user: Dict, base: str, snapshot: Dict[str, Dict[str, Optional[float]]], horizon: str, question: Optional[str] = None) -> str:
    ...
    sys = (
        ...
        "Дай 3–6 осмысленных пунктов: драйверы/риски/наблюдения, строго по запросу. "
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "• 🤖 Помощник — понимает свободный текст и сам запускает нужное действие.\n"
        "• 🧠 Аналитик — краткие инсайты по рынку и ответ на твой вопрос.\n\n"
        "Как пользоваться: нажимай кнопки ниже или просто напиши, например:\n"
        "- цены aapl nvda btc\n"
        "- график TON 30d\n"
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # default: если не в режиме — просто покажем меню
    await update.message.reply_text("Выберите действие:", reply_markup=main_menu_markup(user))
